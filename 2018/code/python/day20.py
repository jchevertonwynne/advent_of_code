...
def construct_map(world):
	min_x = min(world, key=lambda c:c[0])[0]
	min_y = min(world, key=lambda c:c[1])[1]
	max_x = max(world, key=lambda c:c[0])[0]
	max_y = max(world, key=lambda c:c[1])[1]
	return reversed([''.join([world.get((x, y), ' ') for x in range(min_x, max_x + 1)])
